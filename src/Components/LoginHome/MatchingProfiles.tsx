...
import locationIcon from "../../assets/icons/locationIcon.png";
import { ProfileContext } from "../../ProfileContext";
import { fetchSearchProfiles } from "../../commonapicall";
import axios from "axios";
import Pagination from "../Pagination";
import { IoMdArrowDropdown } from "react-icons/io";
import apiClient from "../../API";
...
export const MatchingProfiles = () => {
  ...
  const hasActiveFilters = useCallback(() => {
    return !!(searchProfileId || profession || selectAge || selectedLocation);
  }, [searchProfileId, profession, selectAge, selectedLocation]);

  // Single unified fetch function
  const fetchProfiles = useCallback(async (page: number = paginationValue) => {
    setLoading(true);
    try {
      const result = await fetchSearchProfiles(
        searchProfileId,
      ...
      setTotalCount(result.total_count);
      setMatchingProfileTotalCount(result.total_count);
      sessionStorage.setItem("searchvalue", result.search_result);
      setSearchResult(result.profiles);
      setShouldScroll(true);
  ...
  useEffect(() => {
    if (paginationValue >= 1) {
      fetchProfiles(paginationValue);
    }
  }, [paginationValue, fetchProfiles]);
  ...
  useEffect(() => {
    ...
    const initialAge = urlAge ?? sessionStorage.getItem('selectAge') ?? '';
    const initialLocation = urlLocation ?? sessionStorage.getItem('selectedLocation') ?? '';
    const initialPage = urlPage ? parseInt(urlPage) : parseInt(sessionStorage.getItem('paginationValue') ?? '1');
    ...
    // Only fetch if there are search terms or it's not the first render
    if (initialSearchId || initialProfession || initialAge || initialLocation) {
  ...
  useEffect(() => {
    ...
    const fetchStates = async () => {
      try {
        const response = await apiClient.post("/auth/Get_State_Pref/");
        const statesArray = Object.values(response.data) as State[];
        setStates(statesArray);
      } catch (error) {
        if (axios.isAxiosError(error)) {
    ...
    <div className="max-xl:py-4 max-lg:py-3 bg-grayBg">
      <div className="container mx-auto py-10 max-lg:py-8 max-md:py-6">
        ...
        <div className="bg-white grid grid-cols-6 justify-center items-center rounded-lg gap-4 shadow-profileCardShadow mb-5 px-2 py-[4px] divide-x-[1.5px] divide-ashSecondary max-lg:flex-wrap max-lg:col-gap-5 max-lg:space-x-0 max-xl:grid-cols-3 max-xl:items-end max-sm:grid-cols-1 max-sm:gap-1 max-sm:divide-x-0 max-sm:divide-y-[1px] max-sm:divide-ashSecondary">
          ...
          <div className="relative w-full px-[18px] flex items-center">
            ...
            <select
              ...
              className="w-full text-sm text-primary bg-white pl-3 py-3 cursor-pointer focus-visible:outline-0 appearance-none"
            >
              <option value="" disabled>Age Difference</option>
              {[...Array(10)].map((_, i) => (
        ...
        <div className="flex justify-between items-center pb-3">
          <div className="flex justify-start items-start border-[1px] border-ashSecondary rounded-[4px]">
            <div
              className={`border-r-[1px] border-ashSecondary rounded-l-md p-2 cursor-pointer ${currentView === "gridlist" ? "bg-lightGray" : ""}`}
              title="Gridlist View"
              onClick={() => setCurrentView("gridlist")}
            >
              <HiMiniViewColumns className={`text-[22px] ${currentView === "gridlist" ? "text-secondary" : "text-ashSecondary"} hover:text-secondary`} />
            </div>
            <div
              className={`border-r-[1px] border-ashSecondary p-2 cursor-pointer ${currentView === "list" ? "bg-lightGray" : ""}`}
              title="List View"
              onClick={() => setCurrentView("list")}
            >
              <ImMenu className={`text-[22px] ${currentView === "list" ? "text-secondary" : "text-ashSecondary"} hover:text-secondary`} />
            </div>
            <div
              className={`border-r-none border-ashSecondary p-2 cursor-pointer ${currentView === "grid" ? "bg-lightGray" : ""}`}
              title="Grid View"
              onClick={() => setCurrentView("grid")}
            >
              <BsFillGrid3X3GapFill className={`text-[22px] ${currentView === "grid" ? "text-secondary" : "text-ashSecondary"} hover:text-secondary`} />
            </div>
          </div>
